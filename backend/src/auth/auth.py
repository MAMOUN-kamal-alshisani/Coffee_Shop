...
# ALGORITHMS = ['RS256']
# API_AUDIENCE = 'dev'
# AUTH0_DOMAIN = "dev-f0u7wasufi70tiya.us.auth0.com"
# ALGORITHMS = ['RS256']
...
def verify_decode_jwt(token):
    ...
    if 'kid' not in unverifiedHeader:
        raise AuthError({ 'code': 'invalid header',
                'description': ' token with key id (kid) not found'
                }, 401)
    ...
    if rsa_key:
        try:
            ...
            raise AuthError({
                'code': 'invalid_claims',
                'description': 'invalid claims. check if audience is currect.'
            }, 401)
        except Exception:
            raise AuthError({
                'code': 'invalid_header',
    ...
    raise AuthError({
        'code': 'invalid_header',
                'description': 'Unable to find the currect key.'
    }, 400)
